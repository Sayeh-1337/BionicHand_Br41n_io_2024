...
def knn(query   : tf.TensorSpec(shape = None, dtype = tf.float32) = None,
        ...
        ids : tf.TensorShape((None, )) = None,
        distance_metric = None,
        max_matrix_size = -1,
        
        weighted    : tf.TensorSpec(shape = (), dtype = tf.bool) = False,
    ...
    if ids is None:
        return k_nearest_idx if return_index else tf.gather(
            embeddings, k_nearest_idx, batch_dims = 1
        )
    
    unique_ids, pos_ids = tf.unique(ids)
    
    nearest_ids = tf.gather(pos_ids, k_nearest_idx)

    weights = tf.cond(
        weighted,
        lambda: 1. / tf.maximum(-k_nearest_dists, 1e-9),
        lambda: tf.ones_like(k_nearest_dists)
    )
    scores  = tf.math.bincount(
        nearest_ids,
        weights = weights,
        minlength   = tf.size(unique_ids),
        dtype   = tf.float32,
        axis    = -1
    )

    if return_scores: return unique_ids, scores
    return tf.gather(unique_ids, tf.argmax(scores, axis = -1))
