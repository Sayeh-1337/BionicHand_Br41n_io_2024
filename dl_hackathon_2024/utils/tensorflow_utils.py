...
from utils.generic_utils import get_args, get_kwargs, has_kwargs, convert_to_str
...
def convert_to_tensor(data):
    ...
    elif isinstance(data, bool):                    conv_dtype = tf.bool
...
def cast_arg(value, annot, force = False):
    ...
    if isinstance(annot, tf.TensorSpec):
        ...
        return value
    elif isinstance(annot, tf.TensorShape):
        value = convert_to_tensor(value)
        return tf.ensure_shape(value, annot)
    elif annot == tf.Tensor:
        return convert_to_tensor(value)
    elif force and annot is None and _should_cast_kwarg(value):
        return convert_to_tensor(value)
    return value
...
def tf_compile(fn = None,
    ...
    def wrapper(fn):
        ...
        def log_retracing(args, kwargs):
            if logger.isEnabledFor(getattr(logging, 'RETRACING', logging.DEBUG)):
                log_fn = logger.retracing if hasattr(logger, 'retracing') else logger.debug
                log_fn('Retracing {} with args : {} - kwargs : {}'.format(
                    fn.__name__, args, kwargs
                ))

        def fn_with_retracing_logs(* args, ** kwargs):
            log_retracing(args, kwargs)
        ...
        def inner(* args, run_eagerly = False, use_xla = None, recompile = False, ** kwargs):
            ...
            if recompile or key not in _compiled:
                _compiled[key] = tf.function(
                    fn_with_retracing_logs, jit_compile = use_xla, ** compile_kwargs
                )
        ...
        fn_annots = {
            ...
            if isinstance(v, (tf.TensorShape, tf.TensorSpec)) or v is tf.Tensor
        }
        defaults_to_cast    = {
